...
def send_fight_request(sock, username):
    ...
    request = {"action": "fight_request", "username": username, "boss": boss, "fighting_item": item, "fighting_strength": 3} #request
    response = send_request(sock, request)
    print(f"[Client] {response.get('message')}") #console output
    if "updated_state" in response:
        print("[Client] Your updated state:")
        print(json.dumps(response["updated_state"], indent=2)) #pretty print state

def view_fight_logs(sock):
    request = {"action": "get_fight_logs"} #request logs
    response = send_request(sock, request)
    if response.get("status") == "success":
        print("\n[Client] Confirmed Fight Logs:")
        print("{:<10} {:<10} {:<15} {:<10} {:<10}".format("Requester", "Boss", "Item", "Strength", "Winner")) #header
